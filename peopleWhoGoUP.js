 export  function peopleWhoGoUp(buildingArray, liftCapacity){
            ...
            for(let a = 0; a < floorArray.length; a++){
               ...
               if (floorArray[a] >= i){
                   //ako se u liftu nalazi covek(broj) koji je oznacio da zeli da ide na taj sprat, izlazi iz lifta
                    liftCapacityArray = liftCapacityArray.includes(i) ? liftCapacityArray.filter(item => item !== i) : liftCapacityArray;
                    //ako ima mesta u liftu, osoba koja zeli na gore da ide, ulazi 
                    liftCapacityArray.length < liftCapacity ? liftCapacityArray.push(floorArray[a]) : liftCapacityArray;
                    finalArray.push(i) ;
                }
                else {
                    continue;
